 ...
 *
 * @param <T>    return type of if expression
 * @param <INIT> type of condition of if expression
 * @author wkgcass
 */
public class IfBlock<T, INIT> {
         ...
         * and execute if expression
         *
         * @param func function to invoke with result
         * @return if-expression result
         */
        ...
        public IfBlock<T, INIT> ElseIf(RFunc0<INIT> init, def<T> body) {
                if (procceed)
                        if (initVal != null && !initVal.equals(false)) {
                                return new IfBlock<>(this.body.apply(initVal));
                        } else {
                                return new IfBlock<>(init, body);
                        }
                else
                        return new IfBlock<>(this.val);
        }
